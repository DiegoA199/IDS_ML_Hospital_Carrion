...
('Invitado/demo', 'Acceso limitado para demostración académica');
...
('system.admin', 'Administrar sistema', 'Permite revisar configuración y usuarios', 'sistema');

INSERT OR IGNORE INTO usuarios (username, email, nombre_completo, password_hash) VALUES
...
('invitado', 'invitado@hospital.local', 'Invitado Demo', 'demo-no-produccion');

INSERT OR IGNORE INTO usuarios_roles (usuario_id, rol_id)
SELECT u.id, r.id FROM usuarios u, roles r
WHERE u.username = 'admin' AND r.nombre = 'Administrador TI';

INSERT OR IGNORE INTO usuarios_roles (usuario_id, rol_id)
SELECT u.id, r.id FROM usuarios u, roles r
WHERE u.username = 'analista' AND r.nombre = 'Analista TI';

INSERT OR IGNORE INTO usuarios_roles (usuario_id, rol_id)
SELECT u.id, r.id FROM usuarios u, roles r
WHERE u.username = 'invitado' AND r.nombre = 'Invitado/demo';

INSERT OR IGNORE INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r, permisos p
WHERE r.nombre = 'Administrador TI';

INSERT OR IGNORE INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r, permisos p
WHERE r.nombre = 'Analista TI'
  AND p.codigo IN ('dataset.read', 'dataset.write', 'ml.train', 'ml.predict', 'alerts.manage', 'reports.generate');

INSERT OR IGNORE INTO roles_permisos (rol_id, permiso_id)
SELECT r.id, p.id FROM roles r, permisos p
WHERE r.nombre = 'Invitado/demo'
  AND p.codigo IN ('dataset.read');

INSERT OR IGNORE INTO instituciones (nombre, ruc, tipo, direccion) VALUES
('Hospital Regional Docente Clínico Quirúrgico Daniel Alcides Carrión', '00000000000', 'hospital', 'Huancayo, Perú');

INSERT OR IGNORE INTO sedes (institucion_id, nombre, ciudad, direccion)
SELECT id, 'Sede principal', 'Huancayo', 'Red institucional hospitalaria'
FROM instituciones
WHERE nombre LIKE 'Hospital Regional%';

INSERT OR IGNORE INTO areas_hospitalarias (sede_id, nombre, codigo, descripcion)
SELECT id, 'Unidad TI', 'TI', 'Área responsable de infraestructura y seguridad'
FROM sedes
WHERE nombre = 'Sede principal';

INSERT OR IGNORE INTO cargos (nombre, descripcion) VALUES
('Jefe de TI', 'Responsable de gobierno tecnológico'),
('Analista SOC', 'Responsable de monitoreo y respuesta');

INSERT OR IGNORE INTO responsables_ti (usuario_id, area_id, cargo_id, correo_institucional)
SELECT u.id, a.id, c.id, u.email
FROM usuarios u, areas_hospitalarias a, cargos c
WHERE u.username = 'admin' AND a.codigo = 'TI' AND c.nombre = 'Jefe de TI';

INSERT OR IGNORE INTO tipos_activo (nombre, descripcion) VALUES
('Servidor', 'Servidor institucional'),
('Switch', 'Equipo de conmutación'),
('Firewall', 'Equipo perimetral'),
('Estación clínica', 'Equipo usado en área asistencial');

INSERT OR IGNORE INTO segmentos_red (sede_id, nombre, cidr, criticidad)
SELECT id, 'Segmento SOC demo', '192.168.10.0/24', 'alta'
FROM sedes
WHERE nombre = 'Sede principal';

INSERT OR IGNORE INTO activos_red (tipo_activo_id, area_id, segmento_red_id, nombre, codigo_inventario, criticidad)
SELECT t.id, a.id, s.id, 'Servidor IDS-ML Demo', 'IDSML-SRV-001', 'alta'
FROM tipos_activo t, areas_hospitalarias a, segmentos_red s
WHERE t.nombre = 'Servidor' AND a.codigo = 'TI' AND s.nombre = 'Segmento SOC demo';

INSERT OR IGNORE INTO dispositivos_red (activo_red_id, fabricante, modelo, sistema_operativo, mac_address)
SELECT id, 'Demo', 'Virtual', 'Linux/Windows', '00:00:00:00:00:01'
FROM activos_red
WHERE codigo_inventario = 'IDSML-SRV-001';

INSERT OR IGNORE INTO direcciones_ip (segmento_red_id, activo_red_id, direccion_ip, version_ip)
SELECT s.id, a.id, '192.168.10.10', 'IPv4'
FROM segmentos_red s, activos_red a
WHERE s.nombre = 'Segmento SOC demo' AND a.codigo_inventario = 'IDSML-SRV-001';

INSERT OR IGNORE INTO protocolos_red (nombre, numero_protocolo, descripcion) VALUES
('TCP', 6, 'Transmission Control Protocol'),
('UDP', 17, 'User Datagram Protocol'),
('ICMP', 1, 'Internet Control Message Protocol');

INSERT OR IGNORE INTO servicios_red (protocolo_red_id, nombre, puerto, descripcion)
SELECT id, 'HTTPS', 443, 'Servicio web seguro'
FROM protocolos_red
WHERE nombre = 'TCP';

INSERT OR IGNORE INTO clases_trafico (nombre, descripcion, es_benigna) VALUES
('normal', 'Tráfico sin amenaza identificada', 1),
('DDoS', 'Denegación de servicio distribuida', 0),
('PortScan', 'Escaneo de puertos', 0),
('BruteForce', 'Intento de fuerza bruta', 0);

INSERT OR IGNORE INTO tipos_modelo_ml (nombre, descripcion, requiere_escalamiento) VALUES
...
('Naive Bayes', 'Clasificador probabilístico bayesiano', 0);

INSERT OR IGNORE INTO niveles_severidad (nombre, peso, descripcion) VALUES
...
('Crítica', 4, 'Amenaza confirmada o de alto impacto');

INSERT OR IGNORE INTO tipos_amenaza (nombre, descripcion, categoria) VALUES
...
('Malware', 'Comportamiento asociado a software malicioso', 'Integridad');

INSERT OR IGNORE INTO estados_alerta (nombre, descripcion) VALUES
('nueva', 'Alerta generada y pendiente de revisión'),
('revisada', 'Alerta evaluada por personal TI'),
('cerrada', 'Alerta cerrada con atención documentada');

INSERT OR IGNORE INTO acciones_recomendadas (tipo_amenaza_id, nombre, descripcion, prioridad)
SELECT id, 'Aislar equipo sospechoso', 'Validar origen/destino y aislar temporalmente el activo si procede', 'alta'
FROM tipos_amenaza
WHERE nombre = 'DDoS';

INSERT OR IGNORE INTO tipos_reporte (nombre, descripcion, formato) VALUES
('Resumen ejecutivo IDS-ML', 'Reporte de métricas, alertas y trazabilidad', 'pdf'),
('Experimentos ML', 'Reporte tabular de comparación de modelos', 'csv');

INSERT OR IGNORE INTO configuracion_sistema (clave, valor, descripcion, es_secreto) VALUES
('IDSML_PERSISTENCE_BACKEND', 'auto', 'Backend de persistencia activo', 0),
('MAX_ALERTS_PER_BATCH', '100', 'Máximo de alertas generadas por lote', 0);

INSERT OR IGNORE INTO normas_referencia (codigo, nombre, descripcion, version) VALUES
('ISO27001', 'ISO/IEC 27001', 'Sistema de gestión de seguridad de la información', '2022'),
('NIST-CSF', 'NIST Cybersecurity Framework', 'Marco de referencia de ciberseguridad', '2.0');

INSERT OR IGNORE INTO controles_cumplimiento (norma_referencia_id, codigo_control, descripcion, aplicabilidad)
SELECT id, 'A.8.16', 'Monitoreo de actividades', 'Trazabilidad de eventos IDS-ML'
FROM normas_referencia
WHERE codigo = 'ISO27001';

INSERT OR IGNORE INTO test_plan
...
('CP-10', 'Reportes', 'Validar exportación de reporte.', 'Usabilidad', 'ISO 9001', 'Registros existentes', 'Se descarga un CSV legible.', '', 'Pendiente', 'Analista TI', '2026-06-30', 'Abrir exportación.');

INSERT OR IGNORE INTO literature_implementation
